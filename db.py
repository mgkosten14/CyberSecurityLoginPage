...
class Db:
    ...
    INSERT_ACCOUNT = """INSERT INTO account
        (id, uname, pw_hash, created_at, last_login, accounting, engineering, time, reports)
        VALUES("{id}", "{uname}", "{pw_hash}", "{created_at}", 
        "{last_login}", {accounting}, {engineering}, {time}, {reports});"""
    ...
    @staticmethod
    def execute_query(cnx: sqlite3.Connection, query: str, params: tuple = ()):
        """
        ...
        :param query: str
        :param params: tuple of values to bind to placeholders in the query
        """
        c = cnx.cursor()
